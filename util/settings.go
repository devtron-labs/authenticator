...
func InitialiseSettings(k8sClient *client.K8sClient) error {
	...
	if oldPassword, ok := secret.Data[client.ADMIN_PASSWORD]; !ok || len(oldPassword) == 0 {
		...
		err = kubeutil.CreateOrUpdateSecretField(dexConfig.DevtronDefaultNamespaceName, dexConfig.DevtronSecretName, client.ADMIN_PASSWORD, initialPassword)
		if err != nil {
			return err
		}
		newPassword = true
	...
	err = kubeutil.CreateOrUpdateSecret(dexConfig.DevtronDefaultNamespaceName, dexConfig.DevtronSecretName, func(s *v1.Secret, new bool) error {
		if s.Data == nil {
